...
public class SysPermissionController {
    ...
    @Autowired
    private ISysPermissionProjectService sysPermissionProjectService;
	...
	public Result getSystemSubmenuBatch(@RequestParam("parentIds") String parentIds) {
		try {
			...
			query.orderByAsc(SysPermission::getSortNo);
			List<SysPermission> list = sysPermissionService.list(query);
			Map<String, List<SysPermissionTree>> listMap = new HashMap<>(2000);
...
//	}
	...
	public Result<?> getUserPermissionByToken(@RequestParam(name = "projectIdCookie", required = false) String projectIdCookie) {
		...
		try {
			...
			// 添加首页路由
			// update-begin-author:taoyanmetaList = {ArrayList@21926} size = 184
			// date:20200211 for: TASK #3368 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
			if (!PermissionDataUtil.hasIndexPage(metaList)) {
				SysPermission indexMenu = sysPermissionService
						.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页")).get(0);
			...
			this.getAuthJsonArray(authjsonArray, metaList, projectIdCookie);
			// 查询所有的权限
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
	...
	public Result<Map<String, Object>> queryTreeList() {
		...
		try {
			...
			Map<String, Object> resMap = new HashMap<>(2000);
			// 全部树节点数据
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryProjectRolePermission", method = RequestMethod.GET)
	public Result<List<String>> queryProjectRolePermission(@RequestParam(name = "roleId", required = true) String roleId, @RequestParam(name = "projectId", required = true) String projectId) {
		Result<List<String>> result = new Result<>();
		List<SysPermissionProject> list = new ArrayList<>();
		try {
			list = sysPermissionProjectService.list(new QueryWrapper<SysPermissionProject>().lambda()
					.eq(SysPermissionProject::getRoleId, roleId).eq(SysPermissionProject::getProjectId, projectId));
			result.setResult(list.stream().map(sysPermissionProject -> String.valueOf(sysPermissionProject.getPermissionId()))
					.collect(Collectors.toList()));
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/saveProjectRolePermission", method = RequestMethod.POST)
	/** @RequiresRoles ({"admin"}) */
	public Result<String> saveProjectRolePermission(@RequestBody JSONObject json) {
		long start = System.currentTimeMillis();
		Result<String> result = new Result<>();
		try {
			...
			String permissionIds = json.getString("permissionIds");
			String lastPermissionIds = json.getString("lastpermissionIds");
			this.sysRolePermissionService.saveProjectRolePermission(roleId, permissionIds, lastPermissionIds, projectId);
			result.success("保存成功！");
			log.info("======角色授权成功=====耗时:" + (System.currentTimeMillis() - start) + "毫秒");
		} catch (Exception e) {
			result.error500("授权失败！");
	 ...
	 * @param metaList
	 */
	private void getAuthJsonArray(JSONArray jsonArray, List<SysPermission> metaList, String projectIdCookie) {
		List<String> parentIds = new ArrayList<>();
		JSONArray jsonArraySys = new JSONArray();
		for (SysPermission permission : metaList) {
			...
			if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)//按钮权限
					&& CommonConstant.STATUS_1.equals(permission.getStatus())) {//按钮有效
				json = new JSONObject();
				json.put("action", permission.getPerms());
				json.put("type", permission.getPermsType());
				json.put("describe", permission.getName());
				jsonArraySys.add(json);
			}
		}
		if(!StringUtils.isEmpty(projectIdCookie)) {//以项目按钮权限为准
			LoginUser sysUser = (LoginUser)SecurityUtils.getSubject().getPrincipal();// 获取当前登录用户
			List<SysPermission> sysPermissionList = sysPermissionProjectService.getProjectPermission(parentIds, projectIdCookie, sysUser.getId());
			List<SysPermission> resultList = new ArrayList<>();
			for (SysPermission sysPermission : metaList) {//去除项目按钮权限相关的系统按钮权限
				for (SysPermission sysProjectPermission : sysPermissionList) {
					if(!sysPermission.getParentId().equals(sysProjectPermission.getParentId()) //不是项目相关按钮权限
							&& sysPermission.getMenuType().equals(CommonConstant.MENU_TYPE_2)//按钮权限
								&& CommonConstant.STATUS_1.equals(sysPermission.getStatus())) {//按钮有效
						resultList.add(sysPermission);
					}
				}
			}
			//合并集合
			resultList.addAll(sysPermissionList);
			JSONObject json = null;
			for (SysPermission sysPermission : resultList) {
				json = new JSONObject();
				json.put("action", sysPermission.getPerms());
				json.put("type", sysPermission.getPermsType());
				json.put("describe", sysPermission.getName());
				jsonArray.add(json);
			}
		}else {
			jsonArray.addAll(jsonArraySys);
		}
	}
	...
	public Result<List<String>> queryDepartPermission(
		...
		Result<List<String>> result = new Result<>();
		try {
			List<SysDepartPermission> list = sysDepartPermissionService.list(
					new QueryWrapper<SysDepartPermission>().lambda().eq(SysDepartPermission::getDepartId, departId));
			result.setResult(
					list.stream().map(sysDepartPermission -> String.valueOf(sysDepartPermission.getPermissionId()))
							.collect(Collectors.toList()));
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	 ...
	 * 若为项目角色则查询管理员全部的页面权限
	 */
	@RequestMapping(value = "/queryPagePermission", method = RequestMethod.GET)
	public Result<List<String>> queryPagePermission(@RequestParam(name = "roleId", required = true) String roleId) {
		Result<List<String>> result = new Result<>();
		// 查看此roleId是否是项目角色
		if ("projectRole".equals(sysRoleService.getRoleGroupingById(roleId))) {
			try {
				// 查询管理员角色(默认拥有全部页面权限)的所有权限
				List<String> permissionList = sysRolePermissionService.queryPagePermission("admin");
