...
//			// TODO URL规则有问题？
	...
	public Map<String, String> getDepNamesByUserIds(List<String> userIds) {
		...
		Map<String, String> res = new HashMap<>(2000);
		list.forEach(item -> {
...
//		this.save(user);  //保存角色的时候已经添加过一次了
	...
	public Result<?> checkUserIsEffective(SysUser sysUser) {
		...
		//情况4：用户注销判断,和上面情况2不同,这里用status字段判断,status=3表示注销
		if (CommonConstant.USER_LOGOUT.equals(sysUser.getStatus())) {
			baseCommonService.addLog("用户登录失败，用户名:" + sysUser.getUsername() + "已注销！", CommonConstant.LOG_TYPE_1, null);
			result.error500("该用户已注销,请联系管理员启用");
			return result;
		}
		//情况5：用户禁用逻辑判断,status=4表示禁用
		if (CommonConstant.USER_DISABLE.equals(sysUser.getStatus())) {
			baseCommonService.addLog("用户登录失败，用户名:" + sysUser.getUsername() + "已禁用！", CommonConstant.LOG_TYPE_1, null);
			result.error500("该用户已禁用,请联系管理员解禁");
			return result;
		}
		return result;
	}
	...
	@Override
	public List<String> getSelectUser(String projectId, String roleId) {
		return userMapper.getSelectUser(projectId, roleId);
	}
