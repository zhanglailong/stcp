...
public class SysRolePermissionServiceImpl extends ServiceImpl<SysRolePermissionMapper, SysRolePermission> implements ISysRolePermissionService {
	...
	@Autowired
	ISysPermissionProjectService sysPermissionProjectService;
	...
	public void saveProjectRolePermission(String roleId, String permissionIds, String lastPermissionIds, String projectId) {
		...
		List<String> add = getDiff(lastPermissionIds,permissionIds);
		if(add!=null && add.size()>0) {
			List<SysPermissionProject> list = new ArrayList<SysPermissionProject>();
			for (String p : add) {
				if(oConvertUtils.isNotEmpty(p)) {
					SysPermissionProject rolepms = new SysPermissionProject(roleId, p, projectId);
					rolepms.setOperateDate(new Date());
					rolepms.setOperateIp(ip);
					list.add(rolepms);
				}
			}
			sysPermissionProjectService.saveBatch(list);
		}
		
		List<String> delete = getDiff(permissionIds,lastPermissionIds);
		if(delete!=null && delete.size()>0) {
			for (String permissionId : delete) {
				sysPermissionProjectService.remove(new QueryWrapper<SysPermissionProject>().lambda().eq(SysPermissionProject::getRoleId, roleId).eq(SysPermissionProject::getPermissionId, permissionId));
			}
		}
	}
	...
	public List<String> queryPagePermission(String roleCode) {
		return sysRolePermissionMapper.queryPagePermission(roleCode);
	}
