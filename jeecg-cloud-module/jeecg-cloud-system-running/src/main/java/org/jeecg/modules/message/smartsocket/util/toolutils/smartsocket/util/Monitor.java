 ...
 * @Author: test
 * */
public final class Monitor<T> extends AbstractPlugin<T> implements Runnable {
    ...
    public void run() {
		...
		/*
		 * logger.info("\r\n-----" + seconds + "seconds ----\r\ninflow:\t\t" + curInFlow
		 * * 1.0 / (1024 * 1024) + "(MB)" + "\r\noutflow:\t" + curOutFlow * 1.0 / (1024
		 * * 1024) + "(MB)" + "\r\nprocess fail:\t" + curDiscardNum +
		 * "\r\nprocess success:\t" + curProcessMsgNum + "\r\nprocess total:\t" +
		 * totleProcessMsgNum.longValue() + "\r\nread count:\t" + getAndReset(readCount)
		 * + "\twrite count:\t" + getAndReset(writeCount) + "\r\nconnect count:\t" +
		 * connectCount + "\r\ndisconnect count:\t" + disConnectCount +
		 * "\r\nonline count:\t" + onlineCount + "\r\nconnected total:\t" +
		 * getAndReset(totalConnect) + "\r\nRequests/sec:\t" + curProcessMsgNum * 1.0 /
		 * seconds + "\r\nTransfer/sec:\t" + (curInFlow * 1.0 / (1024 * 1024) / seconds)
		 * + "(MB)");
		 */
    }
