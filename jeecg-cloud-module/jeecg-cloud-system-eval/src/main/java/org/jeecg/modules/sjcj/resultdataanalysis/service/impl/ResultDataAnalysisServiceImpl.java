...
public class ResultDataAnalysisServiceImpl extends ServiceImpl<ResultDataAnalysisMapper, ResultDataAnalysis> implements IResultDataAnalysisService {
    ...
    public ResultDataAnalysis resolver(ResultDataAnalysis resultDataAnalysisInput) throws Exception {
        ...
        if (StringUtils.equals(TOOLID.get(0), resultDataAnalysisInput.getToolId())) {
            ...
            // 找result下一级目录名
            // 调用方法，取目录
            Optional.ofNullable(resultDataAnalysisInput).ifPresent(u -> {
                String data = null;
                try {
...
//                    if (StringUtils.equals(testItem.getEnglishName(), GRAPHMAXIMUM)) { result.put(testItem.getAbbr(), graph.getGraphMaximum()); }
                    if (StringUtils.equals(testItem.getEnglishName(), AVERAGE_RESPONSE)) { if (actionTransaction.isPresent()) { resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), actionTransaction.get().getAverage(),new Date())); } }
                    if (StringUtils.equals(testItem.getEnglishName(), MAXIMUM)) { resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), actionTransaction.get().getMaxiMum(),new Date())); }
                    if (StringUtils.equals(testItem.getEnglishName(), DURATION)) { Object o = summary.get(DURATION);resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), o.toString(),new Date())); }
                    if (StringUtils.equals(testItem.getEnglishName(), AVERAGE_TURNOVER)) { Object o = summary.get(AVERAGE_TURNOVER);resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), actionTransaction.get().getAverage(),new Date())); }
                    if (StringUtils.equals(testItem.getEnglishName(), AVERAGE)) { resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), graph.getGraphAverage(),new Date())); }
                    if (StringUtils.equals(testItem.getEnglishName(), GRAPHMAXIMUM)) { resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(), graph.getGraphMaximum(),new Date())); }
...
//                if(StringUtils.equals(testItem.getEnglishName(),AVERAGERESPONSE)){ result.put(testItem.getAbbr(),jsonObject.getString(AVERAGERESPONSE)); }
                if(StringUtils.equals(testItem.getEnglishName(),WAITINGTIME)){ resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(),jsonObject.getString(WAITINGTIME),new Date())); }
                ...
                if(StringUtils.equals(testItem.getEnglishName(),AVERAGETURNOVER)){ resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(),jsonObject.getString(AVERAGETURNOVER),new Date())); }
                if(StringUtils.equals(testItem.getEnglishName(),TRANSCATION)){ resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(),jsonObject.getString(TRANSCATION),new Date())); }
                if(StringUtils.equals(testItem.getEnglishName(),AVERAGERESPONSE)){ resultDataAnalysisKvList.add(new ResultDataAnalysisKv(resultDataAnalysisInput.getId(),testItem.getAbbr(),jsonObject.getString(AVERAGERESPONSE),new Date())); }
            });
            // 添加结果
            resultDataAnalysisInput.setResultData(result.toString());
            // JMeterJS
