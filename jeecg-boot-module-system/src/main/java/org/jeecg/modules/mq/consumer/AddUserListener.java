...
@Slf4j
@RabbitComponent(value = "addUserListener")
    ...
    public void onMessage(BaseMap baseMap, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) {
        ...
        try {
            for (SysSecretKey sysSecretKey : sysSecretKeyList) {
                ...
                restTemplate.postForObject(addUrl, secret, String.class);
            }
        } catch (RestClientException e) {
            log.error("新增用户信息推送失败==>推送地址:{},推送数据:{}", urlMsg, baseMap);
            e.printStackTrace();
        }
        try {
            channel.basicAck(deliveryTag, false);
        } catch (IOException e) {
            log.error("新增用户消息确认失败==>推送地址:{},推送数据:{}", urlMsg, baseMap);
            e.printStackTrace();
        }
    }
}
