...
@Slf4j
@RabbitComponent(value = "updateUserListener")
@RabbitListener(queues = "updateUserQueue")
/**
 * @Author: test
 * */
public class UpdateUserListener extends BaseRabbiMqHandler<BaseMap> {
    ...
    public void onMessage(BaseMap baseMap, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag) {
        ...
        try {
            for (SysSecretKey sysSecretKey : sysSecretKeyList) {
                String updateUrl = sysSecretKey.getApiUpdateUrl();
                urlMsg = updateUrl;
                ...
                restTemplate.postForObject(updateUrl, secret, String.class);
            }
        } catch (RestClientException e) {
            log.error("更新用户信息推送失败==>推送地址:{},推送数据:{}", urlMsg, baseMap);
            e.printStackTrace();
        }
        try {
            channel.basicAck(deliveryTag, false);
        } catch (IOException e) {
            log.error("更新用户消息确认失败==>推送地址:{},推送数据:{}", urlMsg, baseMap);
            e.printStackTrace();
        }
    }
