...
public class DuplicateCheckController {
	...
	@Autowired
	private ISysDuplicateCheckService sysDuplicateCheckService;
	...
	/**
	 * 校验数据是否在系统中是否存在(适用于多数据源)
	 *
	 * @return
	 */
	@RequestMapping(value = "/newCheck", method = RequestMethod.GET)
	@ApiOperation("重复校验接口")
	public Result<Object> doDuplicateNewCheck(DuplicateCheckVo duplicateCheckVo, HttpServletRequest request) {
		Long num = null;

		log.info("----duplicate check------："+ duplicateCheckVo.toString());

		num = sysDuplicateCheckService.getNum(duplicateCheckVo);
