...
import org.jeecg.modules.mq.model.SysUserModel;
...
public class SysUserController {
	...
	public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
		...
		try {
            ...
            //向其他系统同步用户数据
            SysUserModel sysUserModel = convertToSysUserModel(user, "add");
            BaseMap map = JSONObject.parseObject(JSON.toJSONString(sysUserModel), BaseMap.class);
            rabbitMqClient.sendMessage("addUserQueue",map);
			result.success("添加成功！");
		} catch (Exception e) {
   ...
   /** @RequiresPermissions ("user:edit")*/
	...
	public Result<SysUser> edit(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			if(sysUser==null) {
                ...
                //向其他系统同步用户数据
                SysUserModel sysUserModel = convertToSysUserModel(user, "update");
                BaseMap map = JSONObject.parseObject(JSON.toJSONString(sysUserModel), BaseMap.class);
                rabbitMqClient.sendMessage("updateUserQueue",map);
				result.success("修改成功!");
			}
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
        ...
        BaseMap map = JSONObject.parseObject(JSON.toJSONString(sysUserModel), BaseMap.class);
        rabbitMqClient.sendMessage("deleteUserQueue",map);
	...
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
        ...
        List<SysUser> sysUserList = sysUserService.lambdaQuery().in(SysUser::getId, userIds).list();
		...
		try {
			...
			for (String id : arr) {
				if(oConvertUtils.isNotEmpty(id)) {
                    ...
                    sysUserModel.setUsername(sysUser.getUsername());
                    //写死了
                    if(CommonConstant.USER_UNFREEZE.equals(Integer.valueOf(status))){
                        sysUserModel.setStatus(CommonConstant.USER_UNFREEZE);
                        sysUserModel.setStatusName("解冻");
                        //dq add 解冻操作需要删除redis中记录密码输入错误次数的key
                        redisUtil.del("CURRENT_USER_PASSWORD_MISTAKE_COUNT" + sysUser.getId());
                    }
                    if(CommonConstant.USER_FREEZE.equals(Integer.valueOf(status))){
                        sysUserModel.setStatus(CommonConstant.USER_FREEZE);
                        sysUserModel.setStatusName("冻结");
                    }
                    BaseMap map = JSONObject.parseObject(JSON.toJSONString(sysUserModel), BaseMap.class);
                    rabbitMqClient.sendMessage("updateUserQueue",map);
				}
   ...
   /** @RequiresPermissions ("user:import")*/
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                ...
                for (int i = 0; i < listSysUsers.size(); i++) {
                    ...
                    sysUserExcel.setStatus(CommonConstant.USER_UNFREEZE);
                    try {
                    ...
                    //向其他系统同步用户数据
                    SysUserModel sysUserModel = convertToSysUserModel(sysUserExcel, "add");
                    BaseMap map = JSONObject.parseObject(JSON.toJSONString(sysUserModel), BaseMap.class);
                    rabbitMqClient.sendMessage("addUserQueue",map);
                }
            } catch (Exception e) {
    ...
    /**
     * SysUser 转 SysUserModel
     * @param sysUser
     * @param operation add-新增  update-修改
     * @return
     */
    private SysUserModel convertToSysUserModel(SysUser sysUser,String operation) {
        SysUserModel sysUserModel = new SysUserModel();
        BeanUtils.copyProperties(sysUser, sysUserModel);
        if(CommonConstant.MAN.equals(sysUser.getSex())){
            sysUserModel.setSexName("男");
        }
        if(CommonConstant.WOMAN.equals(sysUser.getSex())){
            sysUserModel.setSexName("女");
        }
        String add="add";
        String update="update";
        if(add.equals(operation)){
            sysUserModel.setStatusName("正常");
        }
        if(update.equals(operation)){
            if(CommonConstant.USER_UNFREEZE.equals(sysUserModel.getStatus())){
                sysUserModel.setStatusName("正常");
            }
            if(CommonConstant.USER_FREEZE.equals(sysUserModel.getStatus())){
                sysUserModel.setStatusName("冻结");
            }
        }
        return sysUserModel;
