...
public class JeecgElasticsearchTemplate {
     ...
     */
    public <T> ResponseEntity<T> dcat(String urlAfter, Class<T> responseType) {
        String url = this.getBaseUrl().append("/dcat").append(urlAfter).append("?").append(FORMAT_JSON).toString();
     ...
     * 查询所有索引
     * <p>
     * 查询地址：GET http://{baseUrl}/dcat/indices
     */
     ...
     * 查询单个索引
     * <p>
     * 查询地址：GET http://{baseUrl}/dcat/indices/{indexName}
     */
    public JSONArray getIndices(String indexName) {
        ...
        return dcat(urlAfter.toString(), JSONArray.class).getBody();
    }
    ...
    public JSONObject getDataById(String indexName, String typeName, String dataId) {
        ...
        if (found) {
            return result.getJSONObject("dsource");
        } else {
            return null;
        }
    }
    ...
    /**
     * @param dsource （源滤波器）指定返回的字段，传null返回所有字段
     * @param query
     * @param from    从第几条数据开始
     * @param size    返回条目数
     * @return { "query": query }
     */
    public JSONObject buildQuery(List<String> dsource, JSONObject query, int from, int size) {
        JSONObject json = new JSONObject();
        if (dsource != null) {
            json.put("dsource", dsource);
        }
