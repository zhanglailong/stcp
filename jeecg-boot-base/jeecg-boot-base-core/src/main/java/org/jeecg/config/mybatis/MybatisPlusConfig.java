...
public class MybatisPlusConfig {
    ...
    static {
        TENANT_TABLE.add("jee_bug_danbiao");
    ...
    private void tenantConfig(PaginationInterceptor paginationInterceptor){
        ...
        tenantSqlParser.setTenantHandler(new TenantHandler() {
            ...
            public boolean doTableFilter(String tableName) {
                ...
                if(TENANT_TABLE.contains(tableName)){
                    return false;
                }
                return true;
            }
        ...
        paginationInterceptor.setSqlParserFilter(new ISqlParserFilter() {
            ...
            public boolean doFilter(MetaObject metaObject) {
                ...
                for(String tableName: TENANT_TABLE){
                    String sqlLowercase  = sql.toLowerCase();
                    if(sqlLowercase.indexOf(tableName.toLowerCase())>=0){
                        for(String key: DDL_KEYWORD){
                            if(sqlLowercase.indexOf(key)>=0){
                                return true;
                            }
                        }
                        return false;
